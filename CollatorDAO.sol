...
contract CollatorDAO is AccessControl{
    ...
    struct NodeConfig{
        ...
        uint fundsUpLimit;//投资抵押上限
        uint perInvestDownLimit;//每人次投资抵押下限
        uint voterProportion;//投票参与人数有效比例
    ...
    constructor(uint _techProportion
        ...
        nodeConfig.fundsUpLimit =  _fundsUpLimit;
        ...
        nodeConfig.authorMappingLimit = _authorMappingLimit;
    ...
    //开启投资抵押上限治理
    function startFULGovern(uint _fundsUpLimit,uint256 _startDate,uint256 _endDate) public isGovernance{
    ...
    //投资抵押上限治理投票
    function voteFULByNumber(uint _number,uint _state) public onlyRole(MEMBER){
        GovernanceInfo memory info = governanceInfos[3];
        info = _setVote(info, _number, _state);
        if(info.success){
            //治理成功
            nodeConfig.fundsUpLimit = info.uintValue;
        }
        governanceInfos[3] = info;
    }
    ...
    function addStake() external payable {
        ...
        require(totalStake + msg.value <= nodeConfig.fundsUpLimit,'More than fundsUpLimit!');
    ...
    function addAssociation(bytes32 nimbusId) external payable isOwner{
        require(governanceFlag,'Node not started!');
        require(address(this).balance - pendingReward >= nodeConfig.authorMappingLimit,'authorMappingLimit is illegal');
